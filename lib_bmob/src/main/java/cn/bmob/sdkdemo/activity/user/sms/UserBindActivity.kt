...
import kotlinx.android.synthetic.main.activity_user_bind.*
 ...
 */
class UserBindActivity : BaseTitleActivity() {
    override fun initVariable(savedInstanceState: Bundle?) {
    }

    override fun initViewAndData() {
        setListener(R.id.btn_send, R.id.btn_bind)
    }

    override fun getLayout(): Int = R.layout.activity_user_bind
        ...
        when (view?.id) {
            R.id.btn_send -> {
                ...
                if (!TextUtils.isEmpty(phone) && verify != null && verify) {
                    ...
                    val phoneInput = edt_phone!!.text.toString().trim { it <= ' ' }
                     ...
                     */
                    BmobSMS.requestSMSCode(phoneInput, "DataSDK", object : QueryListener<Int>() {
            ...
            R.id.btn_bind -> {
                ...
                BmobSMS.verifySmsCode(phone, code, object : UpdateListener() {
                    override fun done(e: BmobException?) {
                        if (e == null) {
                            tv_info!!.append("验证码验证成功，您可以在此时进行绑定操作！\n")
                            val user = BmobUser.getCurrentUser(User::class.java)
                            ...
                            user.update(object : UpdateListener() {
                                override fun done(e: BmobException?) {
                                    if (e == null) {
                                        tv_info!!.append("绑定手机号码成功")
                                    } else {
                                        tv_info!!.append("绑定手机号码失败：" + e.errorCode + "-" + e.message)
                                    }
                                }
                            })
                        } else {
